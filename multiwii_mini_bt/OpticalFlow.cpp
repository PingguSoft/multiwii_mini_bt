...
int8_t optFlowCosZ = 100; // cos(angleZ)*100
...
//  if(accMode ==   1   && optflowMode ==   1   && GPSModeHome ==   0) {
        ...
        if(abs(rcCommand[ROLL]) <   OF_DEADBAND && abs(rcCommand[PITCH]) < OF_DEADBAND) {
            ...
            for(axis=0; axis<2; axis++) {
                ...
                optflow_angle[axis] =   EstHVel[axis]   *   OPTFLOW_P_GAIN / 50;    // 16   bit ok: 100 *   200 =   20000
            }                   
        }   else {
        ...
        for(axis=0; axis<2; axis++) {
            optflow_angle[axis] = constrain(optflow_angle[axis] + (int16_t)((int32_t)optflowErrorI[axis] * OPTFLOW_I_GAIN / 5000),
                -300,   300);
        }
...
inline void optflow_get_vel()   {
    ...
    uint16_t alt10mm;   // alt10mm in   mm*10
    int8_t axis;
    ...
    if(optFlowCosZ >   70 &&   avgSqual.res > 10) {
        // above 3m, freeze altitude (it means less stabilization   on high altitude)
        // ..   and reduce signal   if surface quality <50
        alt10mm =   constrain((int16_t)alt.EstAlt, 30, 300) *   min(avgSqual.res,50) * 2;   // 16   bit ok: 300 *   50 * 2 = 30000;
    }   else {
        alt10mm =   0;
    }
    ...
    for(axis=0;axis<2;axis++)   {
        ...
        if(alt10mm !=   0) { 
            ...
            vel_of[axis] = ((int32_t)optflow_pos[axis] + (att.angle[axis]   -   prevAngle[axis]) * 17   )   *   alt10mm /   dTime   ;   
        }   else {
            vel_of[axis] = 0;
        }
